...
std::array<array<int, 8>, 8> weightmap{ {
		{1200, -300, 150,  50,  50, 150, -300,  1200},
		{-300, -600, -100, -100, -100, -100, -600, -300},
		{150,  -100, 50,   0,   0, 50,  -100,  150},
		{100,  -50,   0,   0,   0,   0,  -50,   100},
		{100,  -50,   0,   0,   0,   0,  -50,   100},
		{150,  -100, 50,   0,   0,  50,  -100,  150},
		{-300, -600, -100, -100, -100, -100, -600, -300},
		{1200, -300, 150,  100,  100, 150, -300,  1200},
	}
};
...
public:
	...
	int setHeuristic() {//temporary set the heuristic in constant.
		...
		if (board[0][0] == player && board[1][1] == player) {
			...
			if (board[1][n - 1] == player || board[0][n - 2] == player)heuristic += 300;
		}
		else if (board[n - 1][0] == player ) {
			if (board[n - 2][1] == player)heuristic += 600;
			if (board[n - 2][0] == player || board[n - 1][1] == player)heuristic += 300;
		}
		else if (board[n - 1][n - 1] == player) {
			if (board[n - 2][n - 2] == player)heuristic += 600;
			if (board[n - 2][n - 1] == player || board[n - 1][n - 2] == player)heuristic += 300;
		}

		int size_of_next_valid_move = next_valid_spots.size();
		heuristic += size_of_next_valid_move * 5;
		int cnt_enemy=0;
		int cnt = 0;
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
	...
	if (now.cur_player == player) {//suppose is the same(so the heuristic will be maximum)
		...
		for (auto test : now.next_valid_spots) {
			...
			if (next.put_disc(test)) {
				...
				if (eval < minEval && depth == MAXDEPTH)cout << record2.x << " " << record2.y << endl;
...
void write_valid_spot(std::ofstream& fout) {
	...
	p = record2;
	// Remember to flush the output to ensure the last action is written to file.//好的
